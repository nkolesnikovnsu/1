...
public class UserResourceImpl {
	...
	public ResponseEntity<String> register(@RequestBody User user) {
		...
		try {
			...
			//user.setRole(roleRepository.findByName(user.getRole().toString()));
			if(user.getEmail().indexOf("helper.com") != -1){
				user.setRole(roleRepository.findByName(ConstantUtils.HELPERADMIN.toString()));
			}
			else{
